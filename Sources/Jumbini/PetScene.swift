...
final class PetScene: SKScene {
    ...
    /// Internal for Wardrobe.swift and TugController.swift — see the note on
    /// the class above.
    let dog = Dog()
    ...
    override func didMove(to view: SKView) {
        ...
        if let stored = UserDefaults.standard.string(forKey: Wardrobe.defaultsKey) {
    ...
    static let toyLingerDuration: TimeInterval = 4
    ...
    func captureJumbini() -> NSImage? {
        ...
        guard let image = CamComposer.compose(
            dogImage: texture.cgImage(), dogPointSize: dogFrame.size, date: .now
        ) else { return nil }
        flashCamFeedback()
        return image
    }
